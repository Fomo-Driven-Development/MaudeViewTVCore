...
// Service wraps active TradingView multi-window control operations.
// Every session-level method routes to a specific chart window via chartID.
type Service struct {
...
// ensurePane activates the given pane index on the specified chart window.
// If pane < 0 it is a no-op (use current active pane).
func (s *Service) ensurePane(ctx context.Context, chartID string, pane int) error {
	if pane < 0 {
		return nil
	}
	status, err := s.cdp.ActivateChartOnChart(ctx, chartID, pane)
	if err != nil {
		return err
	}
...
// --- Chart-level methods (chartID already present; ensurePane now chart-targeted) ---

func (s *Service) GetSymbolInfo(ctx context.Context, chartID string, pane int) (cdpcontrol.SymbolInfo, error) {
	if err := s.ensurePane(ctx, chartID, pane); err != nil {
...
// --- Alerts REST API probe methods (chart-level, chartID already present) ---
...
// --- Indicator dialog / search methods ---
...
func (s *Service) ProbeIndicatorDialogDOM(ctx context.Context, chartID string) (map[string]any, error) {
	return s.cdp.ProbeIndicatorDialogDOMOnChart(ctx, strings.TrimSpace(chartID))
}
...
func (s *Service) ReloadPage(ctx context.Context, chartID, mode string) (cdpcontrol.ReloadResult, error) {
	mode = strings.ToLower(strings.TrimSpace(mode))
	if mode == "" {
		mode = "normal"
	}
	...
	hard := mode == "hard"
	if err := s.cdp.ReloadPage(ctx, strings.TrimSpace(chartID), hard); err != nil {
		return cdpcontrol.ReloadResult{}, err
	}
	return cdpcontrol.ReloadResult{Status: "reloaded", Mode: mode}, nil
}

// --- Session-level methods: routed to specific window via *OnChart variants ---

// --- Watchlist methods ---

func (s *Service) ListWatchlists(ctx context.Context, chartID string) ([]cdpcontrol.WatchlistInfo, error) {
	return s.cdp.ListWatchlistsOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetActiveWatchlist(ctx context.Context, chartID string) (cdpcontrol.WatchlistDetail, error) {
	return s.cdp.GetActiveWatchlistOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) SetActiveWatchlist(ctx context.Context, chartID, id string) (cdpcontrol.WatchlistInfo, error) {
	if err := s.requireNonEmpty(id, "watchlist id"); err != nil {
		return cdpcontrol.WatchlistInfo{}, err
	}
	return s.cdp.SetActiveWatchlistOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id))
}

func (s *Service) GetWatchlist(ctx context.Context, chartID, id string) (cdpcontrol.WatchlistDetail, error) {
	if err := s.requireNonEmpty(id, "watchlist_id"); err != nil {
		return cdpcontrol.WatchlistDetail{}, err
	}
	return s.cdp.GetWatchlistOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id))
}

func (s *Service) CreateWatchlist(ctx context.Context, chartID, name string) (cdpcontrol.WatchlistInfo, error) {
	if err := s.requireNonEmpty(name, "name"); err != nil {
		return cdpcontrol.WatchlistInfo{}, err
	}
	return s.cdp.CreateWatchlistOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(name))
}

func (s *Service) RenameWatchlist(ctx context.Context, chartID, id, name string) (cdpcontrol.WatchlistInfo, error) {
	if err := s.requireNonEmpty(id, "watchlist_id"); err != nil {
		return cdpcontrol.WatchlistInfo{}, err
	}
	if err := s.requireNonEmpty(name, "name"); err != nil {
	...
	return s.cdp.RenameWatchlistOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id), strings.TrimSpace(name))
}

func (s *Service) DeleteWatchlist(ctx context.Context, chartID, id string) error {
	if err := s.requireNonEmpty(id, "watchlist_id"); err != nil {
		return err
	}
	return s.cdp.DeleteWatchlistOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id))
}

func (s *Service) AddWatchlistSymbols(ctx context.Context, chartID, id string, symbols []string) (cdpcontrol.WatchlistDetail, error) {
	if err := s.requireNonEmpty(id, "watchlist_id"); err != nil {
		return cdpcontrol.WatchlistDetail{}, err
	}
	if len(symbols) == 0 {
	...
	return s.cdp.AddWatchlistSymbolsOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id), symbols)
}

func (s *Service) RemoveWatchlistSymbols(ctx context.Context, chartID, id string, symbols []string) (cdpcontrol.WatchlistDetail, error) {
	if err := s.requireNonEmpty(id, "watchlist_id"); err != nil {
		return cdpcontrol.WatchlistDetail{}, err
	}
	if len(symbols) == 0 {
	...
	return s.cdp.RemoveWatchlistSymbolsOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id), symbols)
}

func (s *Service) PinWatchlist(ctx context.Context, chartID, id string) (cdpcontrol.WatchlistInfo, error) {
	if err := s.requireNonEmpty(id, "id"); err != nil {
		return cdpcontrol.WatchlistInfo{}, err
	}
	return s.cdp.PinWatchlistOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id))
}

func (s *Service) FlagSymbol(ctx context.Context, chartID, id, symbol string) error {
	if err := s.requireNonEmpty(id, "watchlist_id"); err != nil {
		return err
	}
	if err := s.requireNonEmpty(symbol, "symbol"); err != nil {
	...
	return s.cdp.FlagSymbolOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(id), strings.TrimSpace(symbol))
}
...
func (s *Service) ListColoredWatchlists(ctx context.Context, chartID string) ([]cdpcontrol.ColoredWatchlist, error) {
	return s.cdp.ListColoredWatchlistsOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) ReplaceColoredWatchlist(ctx context.Context, chartID, color string, symbols []string) (cdpcontrol.ColoredWatchlist, error) {
	...
	return s.cdp.ReplaceColoredWatchlistOnChart(ctx, strings.TrimSpace(chartID), color, symbols)
}

func (s *Service) AppendColoredWatchlist(ctx context.Context, chartID, color string, symbols []string) (cdpcontrol.ColoredWatchlist, error) {
	if !validColors[color] {
		return cdpcontrol.ColoredWatchlist{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "color must be one of: red, orange, green, purple, blue"}
	}
	if len(symbols) == 0 {
	...
	return s.cdp.AppendColoredWatchlistOnChart(ctx, strings.TrimSpace(chartID), color, symbols)
}

func (s *Service) RemoveColoredWatchlist(ctx context.Context, chartID, color string, symbols []string) (cdpcontrol.ColoredWatchlist, error) {
	if !validColors[color] {
		return cdpcontrol.ColoredWatchlist{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "color must be one of: red, orange, green, purple, blue"}
	}
	if len(symbols) == 0 {
	...
	return s.cdp.RemoveColoredWatchlistOnChart(ctx, strings.TrimSpace(chartID), color, symbols)
}

func (s *Service) BulkRemoveColoredWatchlist(ctx context.Context, chartID string, symbols []string) error {
	if len(symbols) == 0 {
		return &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "symbols must not be empty"}
	}
	return s.cdp.BulkRemoveColoredWatchlistOnChart(ctx, strings.TrimSpace(chartID), symbols)
}

// --- Notes methods ---

func (s *Service) ListNotes(ctx context.Context, chartID, symbol string) ([]cdpcontrol.Note, error) {
	return s.cdp.ListNotesOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(symbol))
}

func (s *Service) GetNote(ctx context.Context, chartID string, noteID int) (cdpcontrol.Note, error) {
	if noteID <= 0 {
		return cdpcontrol.Note{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "note_id must be > 0"}
	}
	notes, err := s.cdp.ListNotesOnChart(ctx, strings.TrimSpace(chartID), "")
	if err != nil {
...
func (s *Service) CreateNote(ctx context.Context, chartID, symbolFull, description, title, snapshotUID string) (cdpcontrol.Note, error) {
	...
	return s.cdp.CreateNoteOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(symbolFull), description, title, strings.TrimSpace(snapshotUID))
}

func (s *Service) EditNote(ctx context.Context, chartID string, noteID int, description, title, snapshotUID string) (cdpcontrol.Note, error) {
	if noteID <= 0 {
		return cdpcontrol.Note{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "note_id must be > 0"}
	}
	if err := s.requireNonEmpty(description, "description"); err != nil {
	...
	return s.cdp.EditNoteOnChart(ctx, strings.TrimSpace(chartID), noteID, description, title, strings.TrimSpace(snapshotUID))
}

func (s *Service) DeleteNote(ctx context.Context, chartID string, noteID int) error {
	if noteID <= 0 {
		return &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "note_id must be > 0"}
	}
	return s.cdp.DeleteNoteOnChart(ctx, strings.TrimSpace(chartID), noteID)
}

func (s *Service) TakeServerSnapshot(ctx context.Context, chartID string) (cdpcontrol.ServerSnapshotResult, error) {
	return s.cdp.TakeServerSnapshotOnChart(ctx, strings.TrimSpace(chartID))
}

// --- Pane info ---

func (s *Service) GetPaneInfo(ctx context.Context, chartID string) (cdpcontrol.PanesResult, error) {
	return s.cdp.GetPaneInfoOnChart(ctx, strings.TrimSpace(chartID))
}

// --- Alerts methods (session-level, routed via chartID) ---

func (s *Service) ListAlerts(ctx context.Context, chartID string) (any, error) {
	return s.cdp.ListAlertsOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetAlerts(ctx context.Context, chartID string, ids []string) (any, error) {
	if len(ids) == 0 {
		return nil, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "alert_ids must not be empty"}
	}
	return s.cdp.GetAlertsOnChart(ctx, strings.TrimSpace(chartID), ids)
}

func (s *Service) CreateAlert(ctx context.Context, chartID string, params map[string]any) (any, error) {
	if len(params) == 0 {
		return nil, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "params must not be empty"}
	}
	return s.cdp.CreateAlertOnChart(ctx, strings.TrimSpace(chartID), params)
}

func (s *Service) ModifyAlert(ctx context.Context, chartID string, params map[string]any) (any, error) {
	if len(params) == 0 {
		return nil, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "params must not be empty"}
	}
	return s.cdp.ModifyAlertOnChart(ctx, strings.TrimSpace(chartID), params)
}

func (s *Service) DeleteAlerts(ctx context.Context, chartID string, ids []string) error {
	if len(ids) == 0 {
		return &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "alert_ids must not be empty"}
	}
	return s.cdp.DeleteAlertsOnChart(ctx, strings.TrimSpace(chartID), ids)
}

func (s *Service) StopAlerts(ctx context.Context, chartID string, ids []string) error {
	if len(ids) == 0 {
		return &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "alert_ids must not be empty"}
	}
	return s.cdp.StopAlertsOnChart(ctx, strings.TrimSpace(chartID), ids)
}

func (s *Service) RestartAlerts(ctx context.Context, chartID string, ids []string) error {
	if len(ids) == 0 {
		return &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "alert_ids must not be empty"}
	}
	return s.cdp.RestartAlertsOnChart(ctx, strings.TrimSpace(chartID), ids)
}

func (s *Service) CloneAlerts(ctx context.Context, chartID string, ids []string) error {
	if len(ids) == 0 {
		return &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "alert_ids must not be empty"}
	}
	return s.cdp.CloneAlertsOnChart(ctx, strings.TrimSpace(chartID), ids)
}

func (s *Service) ListFires(ctx context.Context, chartID string) (any, error) {
	return s.cdp.ListFiresOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) DeleteFires(ctx context.Context, chartID string, ids []string) error {
	if len(ids) == 0 {
		return &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "fire_ids must not be empty"}
	}
	return s.cdp.DeleteFiresOnChart(ctx, strings.TrimSpace(chartID), ids)
}

func (s *Service) DeleteAllFires(ctx context.Context, chartID string) error {
	return s.cdp.DeleteAllFiresOnChart(ctx, strings.TrimSpace(chartID))
}

// --- Pine Editor methods ---

func (s *Service) TogglePineEditor(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.TogglePineEditorOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetPineStatus(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.GetPineStatusOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetPineSource(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.GetPineSourceOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) SetPineSource(ctx context.Context, chartID, source string) (cdpcontrol.PineState, error) {
	if err := s.requireNonEmpty(source, "source"); err != nil {
		return cdpcontrol.PineState{}, err
	}
	return s.cdp.SetPineSourceOnChart(ctx, strings.TrimSpace(chartID), source)
}

func (s *Service) SavePineScript(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.SavePineScriptOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) AddPineToChart(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.AddPineToChartOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetPineConsole(ctx context.Context, chartID string) ([]cdpcontrol.PineConsoleMessage, error) {
	return s.cdp.GetPineConsoleOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineUndo(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineUndoOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineRedo(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineRedoOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineNewIndicator(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineNewIndicatorOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineNewStrategy(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineNewStrategyOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineOpenScript(ctx context.Context, chartID, name string) (cdpcontrol.PineState, error) {
	if err := s.requireNonEmpty(name, "name"); err != nil {
		return cdpcontrol.PineState{}, err
	}
	return s.cdp.PineOpenScriptOnChart(ctx, strings.TrimSpace(chartID), name)
}

func (s *Service) PineFindReplace(ctx context.Context, chartID, find, replace string) (cdpcontrol.PineState, error) {
	if find == "" {
		return cdpcontrol.PineState{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "find is required"}
	}
	return s.cdp.PineFindReplaceOnChart(ctx, strings.TrimSpace(chartID), find, replace)
}

func (s *Service) PineGoToLine(ctx context.Context, chartID string, line int) (cdpcontrol.PineState, error) {
	if line < 1 {
		return cdpcontrol.PineState{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "line must be >= 1"}
	}
	return s.cdp.PineGoToLineOnChart(ctx, strings.TrimSpace(chartID), line)
}

func (s *Service) PineDeleteLine(ctx context.Context, chartID string, count int) (cdpcontrol.PineState, error) {
	return s.cdp.PineDeleteLineOnChart(ctx, strings.TrimSpace(chartID), count)
}

func (s *Service) PineMoveLine(ctx context.Context, chartID, direction string, count int) (cdpcontrol.PineState, error) {
	...
	return s.cdp.PineMoveLineOnChart(ctx, strings.TrimSpace(chartID), direction, count)
}

func (s *Service) PineToggleComment(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineToggleCommentOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineToggleConsole(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineToggleConsoleOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineInsertLineAbove(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineInsertLineAboveOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineNewTab(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineNewTabOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PineCommandPalette(ctx context.Context, chartID string) (cdpcontrol.PineState, error) {
	return s.cdp.PineCommandPaletteOnChart(ctx, strings.TrimSpace(chartID))
}

// --- Layout management methods ---

func (s *Service) ListLayouts(ctx context.Context, chartID string) ([]cdpcontrol.LayoutInfo, error) {
	return s.cdp.ListLayoutsOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetLayoutFavorite(ctx context.Context, chartID string) (cdpcontrol.LayoutFavoriteResult, error) {
	return s.cdp.GetLayoutFavoriteOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) ToggleLayoutFavorite(ctx context.Context, chartID string) (cdpcontrol.LayoutFavoriteResult, error) {
	return s.cdp.ToggleLayoutFavoriteOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetLayoutStatus(ctx context.Context, chartID string) (cdpcontrol.LayoutStatus, error) {
	return s.cdp.GetLayoutStatusOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) SwitchLayout(ctx context.Context, chartID string, id int) (cdpcontrol.LayoutActionResult, error) {
	if id <= 0 {
		return cdpcontrol.LayoutActionResult{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "id must be > 0"}
	}
	return s.cdp.SwitchLayoutOnChart(ctx, strings.TrimSpace(chartID), id)
}

func (s *Service) SaveLayout(ctx context.Context, chartID string) (cdpcontrol.LayoutActionResult, error) {
	return s.cdp.SaveLayoutOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) CloneLayout(ctx context.Context, chartID, name string) (cdpcontrol.LayoutActionResult, error) {
	if err := s.requireNonEmpty(name, "name"); err != nil {
		return cdpcontrol.LayoutActionResult{}, err
	}
	return s.cdp.CloneLayoutOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(name))
}

func (s *Service) DeleteLayout(ctx context.Context, chartID string, id int) (cdpcontrol.LayoutActionResult, error) {
	if id <= 0 {
		return cdpcontrol.LayoutActionResult{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "id must be positive"}
	}
	return s.cdp.DeleteLayoutOnChart(ctx, strings.TrimSpace(chartID), id)
}

func (s *Service) RenameLayout(ctx context.Context, chartID, name string) (cdpcontrol.LayoutActionResult, error) {
	if err := s.requireNonEmpty(name, "name"); err != nil {
		return cdpcontrol.LayoutActionResult{}, err
	}
	return s.cdp.RenameLayoutOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(name))
}

func (s *Service) SetLayoutGrid(ctx context.Context, chartID, template string) (cdpcontrol.LayoutStatus, error) {
	if err := s.requireNonEmpty(template, "template"); err != nil {
		return cdpcontrol.LayoutStatus{}, err
	}
	return s.cdp.SetLayoutGridOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(template))
}

func (s *Service) NextChart(ctx context.Context, chartID string) (cdpcontrol.ActiveChartInfo, error) {
	return s.cdp.NextChartOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) PrevChart(ctx context.Context, chartID string) (cdpcontrol.ActiveChartInfo, error) {
	return s.cdp.PrevChartOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) MaximizeChart(ctx context.Context, chartID string) (cdpcontrol.LayoutStatus, error) {
	return s.cdp.MaximizeChartOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) ActivateChart(ctx context.Context, chartID string, index int) (cdpcontrol.LayoutStatus, error) {
	if index < 0 {
		return cdpcontrol.LayoutStatus{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "index must be >= 0"}
	}
	return s.cdp.ActivateChartOnChart(ctx, strings.TrimSpace(chartID), index)
}

func (s *Service) ToggleFullscreen(ctx context.Context, chartID string) (cdpcontrol.LayoutStatus, error) {
	return s.cdp.ToggleFullscreenOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) DismissDialog(ctx context.Context, chartID string) (cdpcontrol.LayoutActionResult, error) {
	return s.cdp.DismissDialogOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) BatchDeleteLayouts(ctx context.Context, chartID string, ids []int, skipActive bool) (cdpcontrol.BatchDeleteResult, error) {
	if len(ids) == 0 {
	...
	var activeID int
	if skipActive {
		status, err := s.cdp.GetLayoutStatusOnChart(ctx, chartID)
		if err == nil {
			layouts, lErr := s.cdp.ListLayoutsOnChart(ctx, chartID)
			if lErr == nil {
	...
	for i, id := range ids {
		...
		_, err := s.cdp.DeleteLayoutOnChart(ctx, chartID, id)
		if err != nil {
...
func (s *Service) PreviewLayout(ctx context.Context, chartID string, id int, takeSnapshot bool) (cdpcontrol.LayoutDetail, error) {
	if id <= 0 {
		return cdpcontrol.LayoutDetail{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "id must be > 0"}
	}

	layouts, err := s.cdp.ListLayoutsOnChart(ctx, chartID)
	if err != nil {
		return cdpcontrol.LayoutDetail{}, err
	}

	var previousID int
	currentStatus, statusErr := s.cdp.GetLayoutStatusOnChart(ctx, chartID)
	if statusErr == nil {
		for _, l := range layouts {
	...
	if !alreadyOnTarget {
		if _, err := s.cdp.SwitchLayoutOnChart(ctx, chartID, id); err != nil {
	...
	if status, err := s.cdp.GetLayoutStatusOnChart(ctx, chartID); err == nil {
	...
	if !alreadyOnTarget && previousID > 0 {
		if _, err := s.cdp.SwitchLayoutOnChart(ctx, chartID, previousID); err != nil {
...
func (s *Service) ListStudyTemplates(ctx context.Context, chartID string) (cdpcontrol.StudyTemplateList, error) {
	return s.cdp.ListStudyTemplatesOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetStudyTemplate(ctx context.Context, chartID string, id int) (cdpcontrol.StudyTemplateEntry, error) {
	if id <= 0 {
		return cdpcontrol.StudyTemplateEntry{}, &cdpcontrol.CodedError{Code: cdpcontrol.CodeValidation, Message: "template_id must be > 0"}
	}
	return s.cdp.GetStudyTemplateOnChart(ctx, strings.TrimSpace(chartID), id)
}
...
func (s *Service) ProbeHotlistsManager(ctx context.Context, chartID string) (cdpcontrol.HotlistsManagerProbe, error) {
	return s.cdp.ProbeHotlistsManagerOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) ProbeHotlistsManagerDeep(ctx context.Context, chartID string) (map[string]any, error) {
	return s.cdp.ProbeHotlistsManagerDeepOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetHotlistMarkets(ctx context.Context, chartID string) (any, error) {
	return s.cdp.GetHotlistMarketsOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetHotlistExchanges(ctx context.Context, chartID string) ([]cdpcontrol.HotlistExchangeDetail, error) {
	return s.cdp.GetHotlistExchangesOnChart(ctx, strings.TrimSpace(chartID))
}

func (s *Service) GetOneHotlist(ctx context.Context, chartID, exchange, group string) (cdpcontrol.HotlistResult, error) {
	...
	return s.cdp.GetOneHotlistOnChart(ctx, strings.TrimSpace(chartID), strings.TrimSpace(exchange), strings.TrimSpace(group))
}

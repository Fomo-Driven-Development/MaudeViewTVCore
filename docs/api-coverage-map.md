...
| Region | Endpoints | What It Does |
|--------|-----------|--------------|
| **Viewport** | `POST zoom`, `POST scroll`, `PUT visible-range`, `GET visible-range`, `POST go-to-date`, `POST reset-view` | Zoom in/out, scroll bars, set exact visible range, jump to date, reset view |
